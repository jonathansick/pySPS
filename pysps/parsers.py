...
class SpecParser(object):
    ...
    def wavelengths(self):
        ...
        if self.specType == "basel":
            lambdaPath = os.path.join(
                fspsDir, "SPECTRA", "BaSeL3.1", "basel.lambda")
        elif self.specType == "miles":
            lambdaPath = os.path.join(
                fspsDir, "SPECTRA", "MILES", "miles.lambda")
        else:
...
class MagParser(object):
    ...
    def __init__(self, magPath):
        ...
        dt = [('age', np.float), ('mass', np.float), ('lbol', np.float),
                ('sfr', np.float)]
        dt += [(name, np.float) for (idx, name, comment) in FILTER_LIST]
        self.data = np.loadtxt(magPath, comments="#", dtype=dt)
        self.data = np.atleast_1d(self.data)  # protect against 1 age results
